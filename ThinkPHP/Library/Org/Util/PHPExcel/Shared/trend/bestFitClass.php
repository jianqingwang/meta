...
class PHPExcel_Best_Fit
{
	...
	protected $_stdchiafResiduals	= 0;
	 ...
	 * @return	 float
	 */
	public function getStdchiafResiduals($dp=0) {
		if ($dp != 0) {
			return round($this->_stdchiafResiduals,$dp);
		}
		return $this->_stdchiafResiduals;
	}	//	function getStdchiafResiduals()
	...
	protected function _calculateGoodnessOfFit($sumX,$sumY,$sumX2,$sumY2,$sumXY,$meanX,$meanY, $const) {
		...
		if ($this->_DFResiduals == 0.0) {
			$this->_stdchiafResiduals = 0.0;
		} else {
			$this->_stdchiafResiduals = sqrt($SSres / $this->_DFResiduals);
		}
		...
		$this->_slopeSE = $this->_stdchiafResiduals / sqrt($SSsex);
		$this->_intersectSE = $this->_stdchiafResiduals * sqrt(1 / ($this->_valueCount - ($sumX * $sumX) / $sumX2));
