			...
			<div class="search-form fr cf">
				<div class="sleft">
					<form name="formSearch" id="formSearch" method="get" name="form1">
						<input type="hidden" name="time_type" value="addtime" />
						<!-- 时间筛选 -->
						...
						<!-- 转入数值范围 -->
						<input type="text" name="num_start" class="search-input form-control" value="<?php echo ($_GET['num_start']); ?>" placeholder="转入数量起" style="width: 100px; float: left; margin-right: 2px;">
						<b style="float: left; margin-right: 2px;">-</b>
						<input type="text" name="num_stop" class="search-input form-control" value="<?php echo ($_GET['num_stop']); ?>" placeholder="转入数量至" style="width: 100px; float: left; margin-right: 10px;">
						...
						<select style="width: 120px; float: left; margin-right: 10px;" name="coinname" class="form-control">
							<option value="" <?php if(empty($_GET['coinname'])): ?>selected<?php endif; ?>>交易币种</option>
							<?php if(is_array($coincx)): $i = 0; $__LIST__ = $coincx;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i; if($vos['title']!='人民币'){ ?>
								<option value="<?php echo ($vos['name']); ?>" <?php if(($vos['name']) == $_GET['coinname']): ?>selected<?php endif; ?>><?php echo ($vos['js_yw']); ?></option>
		...
		<div class="data-table table-striped">
			<table class="">
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><a target="_blank" style="font-size:11px;" href="https://cn.etherscan.com/address/<?php echo ($vo['username']); ?>"><?php echo ($vo['username']); ?></a></td>
							<td><?php echo ($vo['num']*1); ?></td>
							<td><?php echo ($vo['mum']*1); ?></td>
							<td><?php echo ($vo['jyz']*1); ?></td>
							<!--<td><?php if(($vo["cover"]) == "1"): ?>外部转账<?php else: ?>内部转账<?php endif; ?></td>-->
							<td>
								<?php if(($vo["status"]) == "1"): ?>转入成功<?php else: ?>等待处理<?php endif; ?>
							</td>
