...
<style>
	...
	.form-horizontal .controls {overflow: hidden;padding: 5px 5px 5px 0;height: initial;}
p {
    font-size: 14px;
    line-height: 1.72222;
    margin: 0 0 4px;
...
<div id="main-content">
	...
	<div id="main" class="main" style="padding-top:30px;">
		<div class="main-title-h">
			<span class="h1-title">系统配置</span>
		</div>
		<div class="tab-wrap">
			<div class="tab-content">
				<form id="form" action="<?php echo U('Config/index');?>" method="post" class="form-horizontal" enctype="multipart/form-data">
							...
							<table>
								...
								<tr class="controls">
									...
									<td>
										...
										<?php if(!empty($data["app_logos"])): ?><img id="up_img" onclick="getElementById('inputfile').click()" style="cursor:pointer;max-width:100px;" title="点击添加图片" alt="点击添加图片" src="/Upload/logo/<?php echo ($data["app_logos"]); ?>">
										<?php else: ?>
										...
										<input type="hidden" id="img" name="app_logos" value="<?php echo ($data["app_logos"]); ?>">
                              	...
                              	<tr class="controls">
									...
									<td><select class="form-control input-10x" name="xnb_mr">
										<?php if(is_array($coinlist)): $i = 0; $__LIST__ = $coinlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['name']); ?>" <?php if(($vo['name']) == $C['xnb_mr']): ?>selected<?php endif; ?>> <?php echo ($vo['js_yw']); ?>
								...
								<tr class="controls">
									<td class="item-label">币安转入更新时间 :</td>
									<td><input type="text" class="form-control input-10x" name="api_updatetime" value="<?php echo ($C['api_updatetime']*1); ?>" /></td>
									<td class="item-note">分钟</td>
								</tr>-->
								<tr class="controls">
								...
								<tr class="controls">
									<td class="item-label">全网分红比例:</td>
									<td>
										<input type="text" class="form-control input-10x" name="share_ratio" value="<?php echo ($data['share_ratio']); ?>">
									</td>
									<td class="item-note">%</td>
								</tr>
								
								<tr class="controls">
									<td class="item-label">全网分红结算天数:</td>
									<td>

										<input type="text" class="form-control input-10x" name="share_day" value="<?php echo ($data['share_day']); ?>">
									</td>
									<td class="item-note">天</td>
								</tr>
								
								<tr class="controls">
									<td class="item-label">全网分红所需团队算力:</td>
									<td>
										<input type="text" class="form-control input-10x" name="share_team" value="<?php echo ($data['share_team']); ?>">
									</td>
									<td class="item-note">TiB</td>
								</tr>
								
								<tr class="controls">
									<td class="item-label">全网分红所需直推算力:</td>
									<td>
										<input type="text" class="form-control input-10x" name="share_zt" value="<?php echo ($data['share_zt']); ?>">
									</td>
									<td class="item-note">TiB</td>
								</tr>

								<tr class="controls">
									<td class="item-label">全网容量 :</td>
									<td>
										<input type="text" class="form-control input-10x" name="web_num" value="<?php echo ($data['web_num']); ?>">
									</td>
									<td class="item-note"></td>
								</tr>

								<tr class="controls">
									<td class="item-label">XCH 价格 :</td>
									<td>
										<input type="text" class="form-control input-10x" name="node_num" value="<?php echo ($data['node_num']); ?>">
									</td>
									<td class="item-note"></td>
								</tr>

								<tr class="controls">
									<td class="item-label">钱包地址统计 :</td>
									<td>
										<input type="text" class="form-control input-10x" name="addr_num" value="<?php echo ($data['addr_num']); ?>">
									</td>
									<td class="item-note"></td>
								</tr>

								<tr class="controls">
									<td class="item-label">每日营收 :</td>
									<td>
										<input type="text" class="form-control input-10x" name="xchday_num" value="<?php echo ($data['xchday_num']); ?>">
									</td>
									<td class="item-note"></td>
								</tr>

								<tr class="controls">
									<td class="item-label">收获时间（1TiB） :</td>
									<td>
										<input type="text" class="form-control input-10x" name="xchhour_num" value="<?php echo ($data['xchhour_num']); ?>">
									</td>
									<td class="item-note"></td>
								</tr>

								<tr class="controls">
									<td class="item-label">奇亚币总量 :</td>
									<td>
										<input type="text" class="form-control input-10x" name="xch_total" value="<?php echo ($data['xch_total']); ?>">
									</td>
									<td class="item-note"></td>
								</tr>

								<tr class="controls">
									<td class="item-label">收获的奇亚币 :</td>
									<td>
										<input type="text" class="form-control input-10x" name="coin_count" value="<?php echo ($data['coin_count']); ?>">
									</td>
									<td class="item-note"></td>
								</tr>
        ...
        $("#inputfile").change(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        $('#up_img').attr("src", '/Upload/logo/' + data);
                        $('#img').val(data);
                        $('#up_img').show();
                    }
                },
