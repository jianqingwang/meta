...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			<span class="h1-title">团队等级
				 <a class="btn btn-success " href="<?php echo U('tuandui_add');?>">新 增</a>
			</span>
		</div>
		...
		<form id="form" method="post" class="form-horizontal">
			<table class="">
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($vo["ww_zt"]); ?></td>
							<td><?php echo ($vo["ww_td"]); ?></td>
							<td><?php echo ($vo["zt_ren"]); ?></td>
							<td><?php echo ($vo["zs_kj_name"]); ?></td>
							<td><?php echo ($vo['ptfee_fhong']*100); ?> %</td>
							<td>
								<?php if($vo['id']>1){ ?>
								<a href="<?php echo U('tuandui_edit?id='.$vo['id']);?>" class="btn btn-primary btn-xs"><img style="width: 22px;margin-bottom: 3px;margin-right: 4px;" title="编辑" src="/Public/Admin/images/c-ic3.png"/> </a>
...
function status_del(id){
  	layer.open({
      	...
      	yes: function(){
			$.ajax({
				type:'post',
				url:'<?php echo U("Level/tuandui_del");?>',
				data:{'id':id,type:1},
				cache:false,
				dataType:'json',
...
<div style="display:none;">
	...
	<script type="text/javascript" charset="utf-8">
		//导航高亮
		highlight_subnav("<?php echo U('Trade/tuandui');?>");
	</script>