...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			<span class="h1-title"><a href="<?php echo U('Article/index');?>">新闻中心</a></span>
		</div>
		<div class="cf">
			<div class="fl">
			...
			<table class="">
				...
				<tbody>
					<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td><?php if(($vo["sta_mas"]) == "1"): ?>首页提醒<?php else: ?>不提醒<?php endif; ?></td>
								<td>
									<a href="<?php echo U('Article/edit?id='.$vo['id']);?>" class="btn btn-primary btn-xs">
										<img style="width: 22px;margin-bottom: 3px;margin-right: 4px;" title="编辑" src="/Public/Admin/images/c-ic3.png"/>
									</a>
									<?php if($vo['sta_mas']==1){ ?>
									<img onclick="status_no(<?php echo ($vo['id']); ?>)" style="width: 22px;margin-bottom:3px;margin-right: 4px;" title="关闭提醒" src="/Public/Admin/images/zanting.png"/>
									<?php }else{ ?>
									<img onclick="status_ok(<?php echo ($vo['id']); ?>)" style="width: 22px;margin-bottom:3px;margin-right: 4px;" title="开启提醒" src="/Public/Admin/images/start.png"/>
									<?php } ?>
								</td>
...
function status_no(id){
  	layer.open({
      	...
      	yes: function(){
			$.ajax({
				type:'post',
				url:'<?php echo U("Article/tixing_no");?>',
				data:{'id':id,type:1},
				cache:false,
				dataType:'json',
...
function status_ok(id){
  	layer.open({
      	...
      	yes: function(){
			$.ajax({
				type:'post',
				url:'<?php echo U("Article/tixing_ok");?>',
				data:{'id':id,type:1},
				cache:false,
				dataType:'json',
				success:function(data){
