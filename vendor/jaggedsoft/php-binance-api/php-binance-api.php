...
class API {
   ...
   protected $sapi = 'https://api.binance.com/sapi/'; // /< REST endpoint for the withdrawals
   ...
   public function __construct($api_key,$api_secret,array $options = ["useServerTime"=>false], array $proxyConf = null) {
   ...
   public function sell($symbol, $quantity, $price,$type = "LIMIT", array $flags = [] ) {
    ...
    public function avgPrice(string $symbol)
    {
        ...
        return $this->httpRequest("v3/avgPrice", "GET", $parameters, true);
    }
  ...
  // public function history( string $symbol, int $limit = 500, int $fromTradeId = 1 ) {
   ...
   //获取多网络地址
   public function Address($asset,$network) {
      $params = [
      		"sapi" => true,
      		"network" => $network,
            "coin" => $asset
   ...
   //获取币种多网络
   public function networkList($asset,$network) {
      $params = [
      		"sapi" => true
      ];
   ...
   public function withdrawHistory( $asset = null ) {
      $params = [
            "wapi" => true
      ];
      if( is_null( $asset ) == false ) {
   ...
   public function historicalTrades( $symbol ) {
      return $this->tradesData( $this->httpRequest( "v3/historicalTrades", "GET", [
            "symbol" => $symbol,
            "limit"=>15
      ] ) );
   }
   ...
   private function httpRequest( $url, $method = "GET", array $params = [], $signed = false ) {
      ...
      if( $signed == true ) {
         ...
         if( isset( $params[ 'wapi' ] ) ) {
            ...
            unset( $params[ 'sapi' ] );
            $base = 'https://api.binance.com/sapi/';
         }
         $query = http_build_query( $params, '', '&' );
   ...
   private function chartData( $symbol, $interval, array $ticks ) {
      ...
      foreach( $ticks as $tick ) {
         list( $openTime, $open, $high, $low, $close, $assetVolume, $closeTime, $baschialume, $trades, $assetBuyVolume, $takerBuyVolume, $ignored ) = $tick;
         $output[ $openTime ] = [
               "open" => $open,
               "high" => $high,
               "low" => $low,
