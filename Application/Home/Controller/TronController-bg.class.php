...
class TronController extends HomeController
{
  ...
  $p_address = $_GET['p_unknown'];
        ...
        if (!$address) {
            echo json_encode(array('status' => 1, 'info' => 'Connection error'));
            exit();
        }
...
//      $amount=10;
     ...
     $model_user->where(['id' => $user_info['id']])->save(['recharge'=>$user_info['recharge']-$amount]);
