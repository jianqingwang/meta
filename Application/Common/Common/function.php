...
function ChickIsOpen($pid){
    $isopen = 0;
    $pro = M('WeipanProductinfo')->where(array('pid'=>$pid))->find();
    //此时时间
    $_time = time();
    $_zhou = (int)date("w");
    if($_zhou == 0){
    ...
    $_fen = (int)date("i");
    if ($pro['isopen']) {
        $opentime = M('WeipanOpentime')->where('pid='.$pid)->find();
        if($opentime){
        ...
        foreach ($otime_arr as $k => $v) {
            if($k == $_zhou-1){
                ...
                foreach ($_check as $key => $value) {
                    ...
                    if(($_check_shi_1[0]==$_shi && $_check_shi_1[1]<$_fen) || ($_check_shi_1[0]<$_shi && $_check_shi_2[0]>$_shi) || ($_check_shi_2[0]==$_shi && $_check_shi_2[1]>$_fen)){
                        $isopen = 1;
                    }else{
                        $isopen = 0;
                    }
...
function sendMail($to, $title, $content,$MAIL_HOST,$MAIL_USERNAME,$MAIL_PASSWORD,$MAIL_FROM,$MAIL_FROMNAME) {
...
function check($data, $rule = NULL, $ext = NULL)
{
	...
	$validate['mobile'] = '/^[1][3-8]\d{9}$|^([6|9])\d{7}$|^[0][9]\d{8}$|^[6]([8|6])\d{5}$/';
...
class HttpClient {
 ...
 function getname($id){
	$name=M('user')->Field('truename,username')->where("id=".$id)->find();
   	if($name['truename']!=''){
  ...
  function getinfo($id){
	$info=M('user')->Field('id,username,enname,headimg')->where("id=".$id)->find();
	return $info;
}
    // Setter methods
...
// $user = "hsh1317119"; //短信平台帐号
// $pass = "f4481352cd2e6094833872fa098b2781"; //短信平台密码
// $sendurl = $smsapi."sms?u=".$user."&p=".$pass."&m=".$moible."&c=".urlencode($content);
// $result =file_get_contents($sendurl);

	$result = $sendsms->quickPost('http://intlapi.1cloudsp.com/intl/api/v2/send', array(
		'token' => $token,
		'accesskey' => $accesskey,
		'timestamp' => $timestamp,
		'random' => $randomStr,
...
function get_price($coinid,$hbid,$sx){
	//$table = M('Coin_otc')->where("id=$coinid")->getField('name');
	if($sx==1) {
		$res = M('eth')->where("id=$hbid")->getField('price');
	}elseif($sx==0){
		$res = M('eth')->where("id=$hbid")->getField('short_name');
	}elseif($sx==2){
		$res = M('eth')->where("id=$hbid")->getField('name');
	}elseif($sx==3){
		$res = M('eth')->where("id=$hbid")->getField('symbol');
	}
	return $res;
}
